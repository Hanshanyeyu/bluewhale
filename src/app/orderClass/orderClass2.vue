  ...
  <div class="clearfix">
    <div class="menu">
      <el-menu :default-active="defaultActive" :default-openeds="defaultOpeneds" :router="true" class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose" theme="dark">
        <el-submenu v-for="(item,index) in menus" :index="item.name" :key="item.name">
          <template slot="title">{{item.label}}</template>
          <el-menu-item v-for="(subItem,subIndex) in item.children" :key="subItem.name" :index="item.name+'-'+subItem.name" :route="{name: subItem.routerName}">
            {{subItem.label}}
          </el-menu-item>
        </el-submenu>
      </el-menu>
